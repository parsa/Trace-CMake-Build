...
def is_source_file(arg: str) -> bool:
    ...
    lower = arg.lower()
    return any(lower.endswith(ext) for ext in ['.cpp', '.cc', '.cxx', '.c', '.c++'])


def extract_compile_commands(trace: dict[str, Any], build_dir: str | None = None) -> list[dict]:
...
def main():
    ...
    ap.add_argument("trace_json", help="Input trace JSON file (from trace_cmake_build.py)")
