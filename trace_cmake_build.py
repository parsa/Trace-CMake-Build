...
def get_file_strings(exe_path: str) -> Dict[str, str | None]:
    ...
    try:
        ...
        return {
            "description": read_str("FileDescription"),
            "company": read_str("CompanyName"),
        }
    except Exception:
        return {"description": None, "company": None}
...
class ProcessTracer:
    def __init__(
        self, poll_interval: float = 0.01, post_exit_timeout: float = 5.0
    ):
        self.poll_interval = poll_interval
        self.post_exit_timeout = (
            post_exit_timeout  # max time to wait after root exits
        )
    ...
    def _snapshot_process(
        self, proc: psutil.Process, force: bool = False
    ) -> bool:
        ...
        try:
            ...
            if (
                ...
                and not self._is_descendant_of_root(pid)
            ):
                return False
        ...
        if pid not in self.processes:
            ...
            if peak_working_set_bytes is not None:
                rec["peak_working_set_bytes"] = max(
                    ...
                    rec.get("peak_working_set_bytes") or 0,
                )
            if num_page_faults is not None:
                rec["num_page_faults"] = (
                    num_page_faults  # This is cumulative from OS
                )
    ...
    def trace_command(self, cmd: list[str], label: str):
        ...
        while True:
            ...
            for pid in list(self.processes.keys()):
                if (
                    pid not in alive_now
                    and self.processes[pid].get("end_time") is None
                ):
                    self._mark_dead(pid)
    ...
    def to_json(self) -> Dict[str, Any]:
        ...
        return {
            "processes": out,
            "edges": [{"parent": p, "child": c} for p, c in self.edges],
        }


def main():
    ap = argparse.ArgumentParser(
        description="Trace any command's process tree (cross-platform)."
    )
    ap.add_argument(
        "--output", default="processes.json", help="Output JSON file"
    )
    ap.add_argument(
        "--poll-interval",
        type=float,
    ...
    tracer = ProcessTracer(
        ...
        post_exit_timeout=args.post_exit_timeout,
    )
