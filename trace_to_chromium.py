...
def to_trace_events(data: dict, label: str = "") -> list[dict[str, Any]]:
    ...
    for p in processes:
        ...
        if cmdline:
            ...
            args["cmdline"] = cmd_str[:500] + (
                "..." if len(cmd_str) > 500 else ""
            )
        ...
        if p.get("working_set_bytes") is not None:
            args["working_set_MB"] = round(
                p["working_set_bytes"] / 1_048_576, 2
            )
        ...
        if end_ts and p.get("cpu_user_s"):
            cpu_total = (p.get("cpu_user_s") or 0) + (
                p.get("cpu_system_s") or 0
            )
            events.append(
                {
                    "name": "CPU Time",
...
def main():
    ...
    ap.add_argument(
        "inputs", nargs="+", help="Input JSON files from trace_cmake_build.py"
    )
    ap.add_argument(
        "-o",
        "--output",
    ...
    output = {
        ...
        "displayTimeUnit": "ms",
        "metadata": {"source": "trace_cmake_build.py → trace_to_chromium.py"},
    }
